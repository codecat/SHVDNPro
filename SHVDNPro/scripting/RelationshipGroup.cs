...
namespace GTA
{
	public struct RelationshipGroup : IEquatable<RelationshipGroup>, INativeValue
	{
		RelationshipGroup(string name) : this()
		{
			...
			Function.Call(GTA.Hash.ADD_RELATIONSHIP_GROUP, name, hashArg);
		...
		{
			return Function.Call<Relationship>(GTA.Hash.GET_RELATIONSHIP_BETWEEN_GROUPS, Hash, targetGroup);
		}
		...
		{
			Function.Call(GTA.Hash.SET_RELATIONSHIP_BETWEEN_GROUPS, relationship, Hash, targetGroup);

			if (bidirectionally)
			{
				Function.Call(GTA.Hash.SET_RELATIONSHIP_BETWEEN_GROUPS, relationship, targetGroup, Hash);
			}
		}
		...
		{
			Function.Call(GTA.Hash.CLEAR_RELATIONSHIP_BETWEEN_GROUPS, relationship, Hash, targetGroup);

			if (bidirectionally)
			{
				Function.Call(GTA.Hash.CLEAR_RELATIONSHIP_BETWEEN_GROUPS, relationship, targetGroup, Hash);
			}
		}

		public void Remove()
		{
			Function.Call(GTA.Hash.REMOVE_RELATIONSHIP_GROUP, Hash);
		}
