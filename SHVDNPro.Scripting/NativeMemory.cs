	...
	namespace Native
	{
		...
		internal unsafe class EuphoriaMessageTask : IScriptTask
		{
			...
			public void Run()
			{
				...
				{
					IntPtr name = Function.AddStringPool(argument.Key);
					...
					{
						MemoryAccess.SetNmStringAddress(MessageAddress, name, Function.AddStringPool((string)argument.Value));
					}
				}
				...
				if (*(ulong*)(_PedAddress + 48) == (ulong)PedNmAddress && *(float*)(_PedAddress + MinHealthOffset) <= *(float*)(_PedAddress + 640))
				{
					...
					if (GetDelegateForFunctionPointer<FuncUlongIntDelegate>(new IntPtr((long)*(ulong*)(*(ulong*)PedNmAddress + 152)))((ulong)PedNmAddress) != -1)
					{
						...
						{
							IntPtr messagePtr = Function.AddStringPool(_message);
		...
		internal unsafe static class MemoryAccess
		{
			...
			{
				IntPtr handle = Function.AddStringPool(toHash);
			...
			{
				new EuphoriaMessageTask(targetHandle, message, arguments).Run();
			}

			internal static int CreateTexture(string filename)
			{
				return CreateTexture(Function.AddStringPool(filename));
			}
